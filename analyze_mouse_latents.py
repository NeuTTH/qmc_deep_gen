...
def create_aggregated_posterior_heatmap(lattice, posteriors, resolution=200, sigma=5.0):
    """
    ...
    Args:
        ...
        sigma: Gaussian smoothing kernel width (default 5.0 for smooth appearance)
    ...
    # Apply Gaussian smoothing to create smooth appearance (like the paper)
...
def analyze_mouse_latents(
    ...
    Args:
        ...
        n_clusters: Number of clusters (if None, determined by mean-shift)
        batch_size: Batch size for data loading
        n_example_specs: Number of example spectrograms per cluster
        ...
        posterior_sigma: Gaussian smoothing width for aggregated posterior (default 5.0, higher = smoother)
        use_train_data: If True, use training data instead of test data (default False)
                        NOTE: Paper figures might use training data for denser/cleaner visualizations!
    """
    ...
    plt.tight_layout()
    suffix = "_TRAIN" if use_train_data else "_TEST"
    plt.savefig(os.path.join(save_dir, f'figure_E_embedded_latents_by_freq{suffix}.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(save_dir, f'figure_E_embedded_latents_by_freq{suffix}.svg'), bbox_inches='tight')
    print(f"Saved: figure_E_embedded_latents_by_freq{suffix}.png/svg")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'figure_E_embedded_latents_by_length{suffix}.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(save_dir, f'figure_E_embedded_latents_by_length{suffix}.svg'), bbox_inches='tight')
    print(f"Saved: figure_E_embedded_latents_by_length{suffix}.png/svg")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'figure_F_aggregated_posterior{suffix}.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(save_dir, f'figure_F_aggregated_posterior{suffix}.svg'), bbox_inches='tight')
    print(f"Saved: figure_F_aggregated_posterior{suffix}.png/svg")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'figure_G_cluster_examples{suffix}.png'), dpi=300, bbox_inches='tight')
    print(f"Saved: figure_G_cluster_examples{suffix}.png")
    plt.close()
