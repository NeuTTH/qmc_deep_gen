...
from train.losses import gaussian_lp, gaussian_evidence
...
def run_mouse_experiments(save_location, dataloc, train_grid_m=15, test_grid_m=20, n_recons=50, nEpochs=300, max_train_samples=None, train_batch_size=64, test_batch_size=1, print_gpu_mem=False):
    ...
    qmc_model = QMCLVM(latent_dim=qmc_latent_dim, device=device, decoder=decoder_qmc, basis=IdentityBasis())
    ...
    if not os.path.isfile(save_qmc):
        ...
        qmc_model, qmc_opt, qmc_losses = train_qmc.train_loop(qmc_model, train_loader, train_base_sequence.to(device), qmc_loss_function, nEpochs=nEpochs, print_gpu_mem=print_gpu_mem)
    ...
    for ii in range(n_recons):
        ...
        save_fig = os.path.join(save_location, f'qmc_round_trips_sample_{sample_ind}.png')

        sample = test_loader.dataset[sample_ind][0].to(torch.float32).to(device).unsqueeze(0)
        recon_qmc = qmc_model.round_trip(test_base_sequence.to(device), sample, lp_fnc)
        ...
        for ax, label in zip(axs, ['Original', 'QMC reconstruction']):
            ax.set_xticks([])
            ax.set_yticks([])
            ax.set_title(label)
        plt.tight_layout()
