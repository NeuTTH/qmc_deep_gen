"""
inference_latents_agg.py — Run posterior inference using aggregate posterior
clustering (Martinez & Williams 2026, §3.3) instead of Jacobian-based watershed.

Pipeline:
    1. Compute aggregate posterior E_x[p(z|x)] on the lattice
    2. Interpolate onto a dense grid
    3. Density-weighted mean-shift on the lattice to find modes → markers
    4. Watershed on NEGATED aggregate posterior (floods from density peaks)
    5. Jacobian norm computed as diagnostic only

Usage:
    python inference_latents_agg.py \
        --model_path="path/to/checkpoint.tar" \
        --dataloc="path/to/mouse/data" \
        --output_dir="path/to/output/dir" \
        --lattice_m=20 --bandwidth=0.15 \
        --ws_sigma=1.5 --ws_compactness=0.0
"""
...
def toroidal_watershed(
    ...
    Args:
        elevation: (H, W) scalar field. Watershed floods from minima, so
                   ...
                   boundaries. Pass -agg_grid to flood from density peaks.
...
# ---------------------------------------------------------------------------
# Jacobian norm elevation map (diagnostic)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Aggregate posterior grid
# ---------------------------------------------------------------------------

def build_aggregate_posterior_grid(
    lattice_np: np.ndarray,
    aggregated: np.ndarray,
    res: int = 200,
    sigma: float = 1.5,
) -> np.ndarray:
    """Interpolate lattice-evaluated aggregate posterior onto a dense grid.

    The aggregate posterior E_x[p(z|x)] is the QLVM's native density
    estimate (Martinez & Williams 2026, §3.3, Eq. 9). Unlike a sample
    histogram, this reflects the decoder's likelihood landscape and can
    have modes even under a uniform prior.

    Periodic interpolation via 3x3 tiling of lattice points.

    Args:
        lattice_np: (m, 2) lattice coordinates in [0,1]²
        aggregated: (m,) aggregate posterior values at lattice points
        res: output grid resolution
        sigma: Gaussian smoothing sigma (in grid pixels)

    Returns:
        agg_smooth: (res, res) smoothed aggregate posterior, rows=y, cols=x
    """
    pts_tiled = np.vstack([
        lattice_np + np.array([dx, dy])
        for dx in [-1, 0, 1] for dy in [-1, 0, 1]
    ])
    vals_tiled = np.tile(aggregated, 9)

    gx, gy = np.meshgrid(
        np.linspace(0, 1, res),
        np.linspace(0, 1, res),
    )
    agg_grid = griddata(pts_tiled, vals_tiled, (gx, gy), method='cubic')
    agg_grid = np.nan_to_num(agg_grid, nan=0.0)
    agg_grid = np.clip(agg_grid, 0, None)

    if sigma > 0:
        agg_grid = gaussian_filter(agg_grid, sigma=sigma, mode='wrap')

    return agg_grid.astype(np.float32)
...
def plot_watershed(latent_coords, heatmap, ws_labels, centers, save_path,
    ...
    _heatmap_with_boundaries(axes[0, 0], heatmap, ws_labels, centers,
                              ...
                              'Standard watershed — agg. posterior + boundaries')
    ...
    if has_periodic:
        _heatmap_with_boundaries(axes[1, 0], heatmap, ws_labels_periodic, centers,
                                  boundary_levels, xx, yy, vmax,
                                  'Periodic watershed — agg. posterior + boundaries')
        _scatter_by_labels(axes[1, 1], latent_coords, ws_labels_periodic, res, centers,
                           colors, boundary_levels, xx, yy,
...
# ---------------------------------------------------------------------------
# Diagnostic: aggregate posterior vs Jacobian comparison
# ---------------------------------------------------------------------------

def plot_diagnostic_agg_vs_jac(agg_grid, jac_norm, centers, save_path, res=200):
    """Side-by-side comparison: aggregate posterior vs Jacobian norm.

    Confirms whether density valleys align with Jacobian ridges
    (expected per Martinez & Williams 2026, §3.3).
    """
    gx, gy = np.meshgrid(np.linspace(0, 1, res), np.linspace(0, 1, res))
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))

    im0 = axes[0].imshow(agg_grid, origin='lower', extent=(0, 1, 0, 1), cmap='hot')
    axes[0].set_title('Aggregate posterior E[p(z|x)]')
    plt.colorbar(im0, ax=axes[0], fraction=0.046)

    im1 = axes[1].imshow(jac_norm, origin='lower', extent=(0, 1, 0, 1), cmap='hot')
    axes[1].set_title('Jacobian ||∇f||²')
    plt.colorbar(im1, ax=axes[1], fraction=0.046)

    axes[2].imshow(agg_grid, origin='lower', extent=(0, 1, 0, 1), cmap='hot', alpha=0.6)
    axes[2].contour(gx, gy, jac_norm, levels=8, colors='cyan', linewidths=0.8)
    axes[2].set_title('Overlay: posterior + Jacobian contours')
    ...
    for ax in axes:
        ...
        if len(centers) > 0:
            ax.scatter(centers[:, 0], centers[:, 1],
                       c='lime', s=40, marker='x', linewidths=1.5, zorder=10)
...
def run_inference(
    ...
    # Lattice
    lattice_m: int = 20,
    # Clustering (mean-shift on aggregate posterior)
    bandwidth: float = 0.15,
    seed_percentile: float = 30,
    k_neighbors: int = 8,
    res: int = 200,
    ...
    """
    Run inference on mouse USV spectrograms using aggregate posterior clustering.

    Clusters are found automatically by density-weighted mean-shift on the
    aggregate posterior E_x[p(z|x)] (bandwidth controls granularity).
    Bandwidth guidance for [0,1]²:
      h=0.05  → 15-25 clusters (very fine)
      h=0.10  → 10-15 clusters (fine)
      h=0.15  → 6-10 clusters  (moderate, default)
      h=0.25  → 3-5 clusters   (coarse)

    All outputs are saved inside output_dir:
        latents_full_YYYYMMDD.pkl
        latents_full_provenance.json
        arrays.npz
        spec_ids.json
        watershed_check.png
        toroidal_vs_standard.png
        torus_boundary_diagnostic.png
        diagnostic_agg_vs_jac.png
    ...
    # ── Aggregate posterior grid (paper's native density estimate) ────────────
    print('Building aggregate posterior grid...')
    agg_grid = build_aggregate_posterior_grid(
        lattice_np, aggregated, res=res, sigma=ws_sigma,
    )

    # ── Jacobian norm (diagnostic only) ──────────────────────────────────────
    print('Computing decoder Jacobian norm (diagnostic)...')
    jac_norm = compute_jacobian_norm_fd(model, c_dim=c_dim, res=res, device=device)

    # ── Mean-shift clustering on lattice (Eq. 9, paper §3.3/A.2) ────────────
    print(f'Running density-weighted mean-shift (bandwidth={bandwidth})...')
    centers, lattice_labels, seed_mask = run_mean_shift_fast(
        lattice_embedded,
        weights=aggregated,
        bandwidth=bandwidth,
        embedded=True,
    ...
    # run_mean_shift_fast already returns centers in [0,1]²
    n_clusters = len(centers)
    print(f'  Mean-shift found {n_clusters} modes')

    # ── Plant markers at mean-shift modes ────────────────────────────────────
    ...
    # ── Watershed on NEGATED aggregate posterior ─────────────────────────────
    # Negate: watershed floods from minima, density peaks are cluster centers
    ws_labels = watershed(-agg_grid, markers=marker_img, compactness=ws_compactness)
    ...
    ws_labels_periodic = toroidal_watershed(
        -agg_grid, marker_img, compactness=ws_compactness,
    )
    ...
    cluster_sizes = {i + 1: int((sample_ws == i + 1).sum()) for i in range(n_clusters)}
    ...
    provenance_path = os.path.join(save_dir, 'latents_full_provenance.json')
    ...
    np.savez(
        ...
        agg_grid=agg_grid.astype(np.float32),                  # (res, res)
    ...
    # ── Plots ────────────────────────────────────────────────────────────────
    plot_watershed(
        latent_coords, agg_grid, ws_labels, centers,
    ...
    # Diagnostic: aggregate posterior vs Jacobian alignment
    plot_diagnostic_agg_vs_jac(
        agg_grid, jac_norm, centers,
        save_path=os.path.join(save_dir, 'diagnostic_agg_vs_jac.png'),
        res=res,
    )
    ...
    print(f'\nDone. {len(result)} sessions written, {n_clusters} clusters.')
